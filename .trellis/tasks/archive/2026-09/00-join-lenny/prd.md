...
`lenny` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/lenny/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/lenny/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python3 ./.trellis/scripts/task.py list --assignee lenny` to
...
python3 ./.trellis/scripts/task.py archive 00-join-lenny
```
