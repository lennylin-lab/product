...
public class ScheduleJobTimeoutService {
    ...
    public int sweepTimeoutJobsOnce() {
        // 分布式锁防止重复扫描
        boolean locked = redisDistributedLock.tryLock(PlanningLockKeys.Schedule.TIMEOUT_SWEEP_LOCK_KEY, 0, 120, TimeUnit.SECONDS);
        if (!locked) {
            return 0;
        }
        ...
        try {
            ...
            redisDistributedLock.unlock(PlanningLockKeys.Schedule.TIMEOUT_SWEEP_LOCK_KEY);
        }
    }
