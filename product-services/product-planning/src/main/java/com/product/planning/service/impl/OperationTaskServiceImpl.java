...
import com.product.planning.dto.OperationTaskError;
...
import com.product.planning.service.IOperationTaskService;
...
public class OperationTaskServiceImpl extends ServiceImpl<OperationTaskMapper, OperationTask> implements IOperationTaskService {
    ...
    @Autowired
    private SchedulingSnapshotLoader snapshotLoader;
    @Autowired
    ...
    public AjaxResult generateTask(List<Long> batchIds) {
        ...
        CompletableFuture<List<ProductionBatch>> productionBatchFuture = CompletableFuture.supplyAsync(() ->
                        Db.lambdaQuery(ProductionBatch.class)
                                // orderLineId 必须查询：后续 loadOrderLineMap 依赖它
                                // （单体同位置漏查该列导致 orderLine 恒空 —— 冻结缺陷，此处修复）
                                .select(ProductionBatch::getBatchId, ProductionBatch::getOrderLineId,
                                        ProductionBatch::getStatus, ProductionBatch::getBatchQty)
        ...
        // Phase 4：订单行/模具参数/启用路线经契约批量加载（版本化快照通道，见
        // SchedulingSnapshotLoader）；单体三段同库查询改为 batchToOrderLine（本地）+
        // orderLines/orders/products（需求/主数据域各 1 次批量），调用次数与批次数无关。
        Map<Long, OrderLineSnapshot> orderLineMap = loadOrderLineMap(productionBatches);
        Map<Long, ProductMoldParam> productMoldParamMap = loadProductMoldParamMap(orderLineMap.values());
        Map<Long, List<RouteOperation>> routeOperationsByProduct =
                loadActiveRouteOperationsByProductIds(orderLineMap.values());
        // 开始循环添加任务
        for (Long batchId : batchIds) {
            ...
            OrderLineSnapshot orderLine = item.getOrderLineId() == null ? null : orderLineMap.get(item.getOrderLineId());
    ...
    public AjaxResult retryGenerateTask(Long batchId) {
        ...
        ProductionBatch productionBatch = Db.lambdaQuery(ProductionBatch.class)
                // 同 generateTask：orderLineId 修复单体冻结缺陷
                .select(ProductionBatch::getBatchId, ProductionBatch::getOrderLineId,
                        ProductionBatch::getStatus, ProductionBatch::getBatchQty)
        ...
        Map<Long, OrderLineSnapshot> orderLineMap = loadOrderLineMap(List.of(productionBatch));
        Map<Long, ProductMoldParam> productMoldParamMap = loadProductMoldParamMap(orderLineMap.values());
        OrderLineSnapshot orderLine = productionBatch.getOrderLineId() == null
                ? null
    ...
    private BatchTaskGenerationResult buildBatchTasks(ProductionBatch batch,
                                                      ...
                                                      Map<Long, OrderLineSnapshot> orderLineMap,
    ...
    /**
     * 批量加载批次关联的订单行（Phase 4：order_line 归 demand，经契约批量加载；
     * 与单体同库 IN 查询语义一致，调用次数与批次数无关）。
     */
    private Map<Long, OrderLineSnapshot> loadOrderLineMap(Collection<ProductionBatch> batches) {
        ...
        return snapshotLoader.loadOrderLines(orderLineIds);
    }

    /**
     * 批量加载订单行关联产品的启用路线工序（Phase 4：product/route 归 master-data，
     * 经 getProducts 契约 activeRoute.operations 加载，排序与单体
     * ProductRouteQueryService.loadActiveRouteOperationsByProductIds 一致：
     * sequence 升序 → opCode 升序）。
     */
    private Map<Long, List<RouteOperation>> loadActiveRouteOperationsByProductIds(
            Collection<OrderLineSnapshot> orderLines) {
        ...
        Map<Long, Product> products = snapshotLoader.loadProducts(productIds);
        Map<Long, List<RouteOperation>> result = new HashMap<>();
        products.values().forEach(product -> {
            List<RouteOperation> operations = extractActiveOperations(product);
            if (!operations.isEmpty()) {
                result.put(product.getProductId(), operations);
            }
        });
        return result;
    }

    /** 提取产品启用路线的工序定义（loader 已按 sequence 升序 → opCode 升序排好）。 */
    private List<RouteOperation> extractActiveOperations(Product product) {
        return product == null || product.getActiveOperations() == null
                ? List.of()
                : product.getActiveOperations();
    }

    /**
     * 批量加载订单行关联产品的模具参数（Phase 4：product_mold_param 归 master-data，
     * 经 getProducts 契约 moldParams 加载；与单体同库查询语义一致——同产品多条参数时
     * 取第一条，契约返回顺序即 master-data 读取顺序）。
     */
    private Map<Long, ProductMoldParam> loadProductMoldParamMap(Collection<OrderLineSnapshot> orderLines) {
        if (CollectionUtils.isEmpty(orderLines)) {
            return Map.of();
        }
        Set<Long> productIds = orderLines.stream()
        ...
        products.forEach((productId, product) -> {
            if (product.getMoldParams() != null && !product.getMoldParams().isEmpty()) {
                paramMap.putIfAbsent(productId, product.getMoldParams().get(0));
            }
        });
        return paramMap;
    }
    ...
    private long resolveStdDurationMin(String stdTimeModel,
                                       ...
                                       Map<Long, OrderLineSnapshot> orderLineMap,
                                       Map<Long, ProductMoldParam> productMoldParamMap) {
        OrderLineSnapshot orderLine = batch == null || batch.getOrderLineId() == null
                ? null
