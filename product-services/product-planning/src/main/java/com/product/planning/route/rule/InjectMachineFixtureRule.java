...
/**
 * 注塑（夹具感知）规则：机台 + 夹具（2026-09-15 夹具兼容任务新增）。
 *
 * <p>工序启用方式与既有规则一致：{@code route_operation.eligible_resource_rule}
 * 配置为本规则码即产出强制 FIXTURE 需求行（isMandatory=1、resourceId 为 null——
 * 夹具选择发生在排程计算器）；不配置则走既有 INJECT 兜底，旧行为零变化。
 * 兼容性数据经快照 Fixture.moldCompatibilityList（fixture ↔ mold 显式允许清单）供
 * 计算器过滤。</p>
 */
@Component
public class InjectMachineFixtureRule implements RouteEligibleResourceRule {

    @Override
    public String code() {
        return RouteOperationConstants.RULE_INJECT_MACHINE_FIXTURE;
    }
