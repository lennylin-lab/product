...
/**
 * 计划域内部契约端点（product-planning-api 的 PlanningBatchApi 实现，Phase 4）。
 *
 * <p>消费方：product-demand（订单行详情 ProductionBatchView 填充、删除订单行时级联
 * 清理本域批次）；product-master-data（启用工艺路线删除保护的阻塞任务判断）。</p>
 *
...
public class InternalPlanningController implements PlanningBatchApi {
