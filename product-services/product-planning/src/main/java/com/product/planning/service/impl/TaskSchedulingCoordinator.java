...
import com.product.planning.dto.ScheduleExecutionResult;
import com.product.planning.dto.ScheduleProgressDTO;
import com.product.planning.dto.TaskSchedulingPriorityDTO;
import com.product.planning.enums.SchedulePhase;
...
public class TaskSchedulingCoordinator {
    ...
    private ScheduleExecutionResult runScheduleAll(TaskAssignmentDTO taskAssignmentDTO, Consumer<ScheduleProgressDTO> progressConsumer) {
        ...
        CompletableFuture<List<com.product.planning.domain.entity.OperationTask>> readyTasksFuture = CompletableFuture.supplyAsync(
                taskSchedulingQueryService::loadReadyTaskList,
                threadPoolTaskExecutor);

        List<Resource> machines = machinesFuture.join();
        List<com.product.planning.domain.entity.OperationTask> readyTasks = readyTasksFuture.join();
        ...
        List<Long> machineIds = machines == null ? List.of() : machines.stream()
                ...
                .filter(Objects::nonNull)
                .toList();
        Map<Long, com.product.planning.dto.MachineLastAssignmentDTO> machineLastAssignments =
        ...
        Map<Long, TaskSchedulingPriorityDTO> priorityMap = SchedulingStrategy.DUE_DATE_PRIORITY == strategy
                ...
                : Map.of();
        List<com.product.planning.domain.entity.OperationTask> orderedTasks =
                taskSchedulingCalculator.orderTasks(readyTasks, strategy, priorityMap);

        List<Long> schedulableTaskIds = orderedTasks.stream()
                .filter(Objects::nonNull)
                .map(com.product.planning.domain.entity.OperationTask::getTaskId)
                .filter(Objects::nonNull)
                .toList();
        ...
        // 阶段1.5: 版本化快照基线（Phase 4，ADR-0004）：输入加载完成后捕获两域数据版本，
        // 落库前复核（见 verifyUnchanged）——替代单体同库读一致性，规则见 SchedulingSnapshotLoader。
        SchedulingSnapshotLoader.Versions snapshotVersions = snapshotLoader.captureVersions();
        ...
        for (int index = 0; index < orderedTasks.size(); index += scheduleBatchSize) {
            List<com.product.planning.domain.entity.OperationTask> tasks =
        ...
        // 阶段2.5: 快照漂移复核（Phase 4）：计算完成后、事务落库前重新读取两域版本，
        // 任一漂移即以显式异常中止（任务保持 READY，重新发起排程即重试），绝不部分落库。
        snapshotLoader.verifyUnchanged(snapshotVersions);
    ...
    private boolean taskRequiresMachine(com.product.planning.domain.entity.OperationTask task) {
        ...
        return task.getResourceRequirementList().stream()
                ...
                .anyMatch(req -> com.product.planning.common.constant.ResourceConstants.RESOURCE_TYPE_MACHINE
