...
public class OrderLine{
    ...
    /** 生产批次（订单行拆批）信息——planning 域数据（ADR-0005 禁止跨服务持久化模型）：
     *  本服务不再持有 production_batch 实体/表，Phase 3 起响应形状保持（无批次数据时为空列表），
     *  Phase 4 经 product-planning 契约填充。 */
    @TableField(exist = false)
    private List<ProductionBatchView> productionBatchList;
